...
model = GNN_CNN_Saliency(gnn_in_channels=num_node_features, gnn_hidden_channels=32, gnn_out_channels=64, cnn_out_features=512, num_classes=256 * 256)
...
for epoch in range(10):
    ...
    for batch_idx, (images, masks) in enumerate(duts_dataloader):
        ...
        if (batch_idx + 1) % 10 == 0:
            ...
            show_image(images[0], title="Input Image")
            show_image(masks[0].squeeze(0), title="Ground Truth Mask")
            show_image(output[0].sigmoid().detach(), title="Predicted Mask")
